...
function renderDetail(film) {
  ...
  poster.onerror = () => {
    poster.src =
      "https://via.placeholder.com/280x420/222534/888899?text=No+Poster";
  };
  ...
  document.getElementById("detail-duration").textContent = film.duration
    ? `${film.duration} mnt`
    : "—";
  document.getElementById("detail-genre-text").textContent =
    film.genre || "Film";
  ...
  document.getElementById("detail-language").textContent =
    film.language || "Inggris";
  document.getElementById("detail-subtitle").textContent = film.has_subtitle
    ? "Tersedia"
    : "Tidak Tersedia";
...
function renderMoreFilms(films) {
  ...
  if (films.length === 0) {
    grid.innerHTML =
      '<p style="color:var(--muted); font-size:13px;">Tidak ada film lain tersedia.</p>';
    return;
  }
